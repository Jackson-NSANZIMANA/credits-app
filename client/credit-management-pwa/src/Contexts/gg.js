...
import { getDebts, getPayments, savePayment, getCustomers } from '../pages/localStorage'
...
const Payments = () => {
  ...
  const filteredDebts = debts.filter(debt => {
    ...
    const matchesSearch = debtsSearchQuery ? getCustomerName(debt.customerId).toLowerCase().includes(debtsSearchQuery.toLowerCase()) : true
  ...
  const clearDebtsSearch = () => {
    setDebtsSearchQuery('')
  }

  const clearPaymentsSearch = () => {
      ...
      <div className="payments-content content-overlay">
        ...
        <div className="page-header">
          ...
          <div className="header-stats">
            <div className="stat-item rwanda-card">
              <span className="stat-value rwanda-green-text">{getTotalOutstanding().toLocaleString()} RWF</span>
              <span className="stat-label">{t.outstanding}</span>
            </div>
            <div className="stat-item rwanda-card">
              <span className="stat-value rwanda-green-text">{getTotalPaid().toLocaleString()} RWF</span>
        ...
        <div className="filters-card rwanda-card">
          ...
          <div className="search-container">
            <div className="search-input-group">
              ...
              />
              {debtsSearchQuery && (
                <button className="search-clear" onClick={clearDebtsSearch}>
                    ...
                    {filteredDebts.map(debt => (
                      <tr key={debt.id} className={`debt-row debt-${debt.status || 'pending'}`}>
                        ...
                        <td className="debt-amount">
                          {(debt.totalAmount || 0).toLocaleString()} RWF
                        </td>
                        
                        <td>
                          <span className={`status-badge status-${debt.status || 'pending'}`}>
                            {debt.status || t.pending}
                          </span>
                        </td>
              ...
              <div className="payment-summary">
                ...
                <div className="summary-details">
                  ...
                  <div className="summary-row">
                    ...
                    <span>{(selectedDebt.totalAmount || 0).toLocaleString()} RWF</span>
                  </div>
                  <div className="summary-row">
                ...
                <div className="payment-preview">
                  ...
                  <div className="preview-details">
                    <div className="preview-row">
                      ...
                      <span>{(parseFloat(paymentData.amount) || 0).toLocaleString()} RWF</span>
                    </div>
                    <div className="preview-row">
                      <span>{t.remainingBalance}:</span>
                      <span>{((selectedDebt.totalAmount || 0) - (parseFloat(paymentData.amount) || 0)).toLocaleString()} RWF</span>
                    </div>
                  </div>
                </div>
        ...
        <div className="card rwanda-card">
          <div className="card-header">
            ...
            <span className="badge">{filteredPayments.length} {t.payments}</span>
          </div>
          ...
          <div className="search-container">
            <div className="search-input-group">
              ...
              <input
                type="text"
                placeholder={t.searchPaymentsPlaceholder || "Search payments by customer, method or notes..."}
                value={paymentsSearchQuery}
                onChange={(e) => setPaymentsSearchQuery(e.target.value)}
                className="search-input"
              />
              {paymentsSearchQuery && (
                <button className="search-clear" onClick={clearPaymentsSearch}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="#718096">
                    <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
                  </svg>
                </button>
              ...
              <div className="empty-state">
                {paymentsSearchQuery ? (
                  <>
                    ...
                    <p>{t.noPaymentsMatchQuery || "No payments match your search criteria."}</p>
                    <button 
                      className="btn btn-outline"
                      onClick={clearPaymentsSearch}
                    >
                      {t.clearSearch || "Clear search"}
                    </button>
                  </>
                  ...
                  <tbody>
                    {filteredPayments.slice(-10).reverse().map(payment => (
                      <tr key={payment.id}>
                        ...
                        <td>{payment.customerName || t.unknownCustomer}</td>
                        <td className="payment-amount">{(payment.amount || 0).toLocaleString()} RWF</td>
                        <td>
